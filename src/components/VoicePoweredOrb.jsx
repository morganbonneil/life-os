...
// A swirling WebGL "voice orb" — idle-animated purely from time, with an
// optional microphone-reactive mode (unused on the launch splash, which
// passes enableVoiceControl={false} so no mic permission is ever requested
// just to show the animation).
export function VoicePoweredOrb({
    ...
    try {
      ...
      return () => {
        ...
        if (container && glContext && glContext.canvas) {
          try { if (container.contains(glContext.canvas)) container.removeChild(glContext.canvas); } catch { /* ignore */ }
        }
        stopMicrophone();
  ...
  return <div ref={ctnDom} className={"voice-orb" + (className ? " " + className : "")} />;
}
